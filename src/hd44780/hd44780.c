...
static void DELAY(volatile int32u_t us)
{
 ...
 for(alfa=0;alfa<n;alfa++)
 {
	...
	/*
	int32u_t t=0;
	if(us >= 1000)vTaskDelay(us/1000);
	if(us >= 100)vTaskDelay(us/100);
	if(us >= 10)vTaskDelay(us/10);
	if(us < 10)vTaskDelay(1);
	*/
}
...
portBASE_TYPE to_video_mem(uint8_t x, uint8_t y, uint8_t *data){
	portBASE_TYPE xStatus;
	lcd_mem_changed = 1;

	if(xSemaphoreTake(xLcdMutex, portMAX_DELAY) == pdPASS){
		if(y == 1){
			arr_cp(lcd_mem+LCD_LINE_LENGHT+x, data);
		}
		if(y == 0)arr_cp(lcd_mem+x, data);
		xSemaphoreGive(xLcdMutex);
	}
	else return pdFAIL;
	return pdPASS;
...
portBASE_TYPE cln_scr(void){
	portBASE_TYPE xStatus;
	lcd_mem_changed = 1;

	if(xSemaphoreTake(xLcdMutex, portMAX_DELAY) == pdPASS){
		for(uint8_t i=0; i<(LCD_LINE_LENGHT*2); i++)lcd_mem[i] = ' ';
		lcd_mem[LCD_LINE_LENGHT*2] = '\0';
		xSemaphoreGive(xLcdMutex);
	}
	else return pdFAIL;
	return pdPASS;
